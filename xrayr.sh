...
# 移除NodeID:后的空格，然后再添加一个空格到:与数字之间
sed -i 's/NodeID: $node_id/NodeID:$node_id/' "$CONFIG_FILE"
sed -i 's/NodeID:/NodeID: /' "$CONFIG_FILE"

# 再次重启XrayR服务以应用最新的配置更改
echo "应用NodeID格式更改后，正在再次重启XrayR服务..."
xrayr restart
